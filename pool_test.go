...
func mkEntry(label string) keyFileEntry {
	return keyFileEntry{Label: label, Key: "key-" + label}
}

func TestDailyCapExhaustsAndCoolsUntilMidnight(t *testing.T) {
	p := newPool("zen", "priority", 2, []keyFileEntry{mkEntry("a")})
	now := time.Now()
...
func TestDailyCapPriorityFallsOverToNextKey(t *testing.T) {
	p := newPool("zen", "priority", 1, []keyFileEntry{mkEntry("a"), mkEntry("b")})
...
func TestDailyCapLRUAlternates(t *testing.T) {
	p := newPool("zen", "lru", 1, []keyFileEntry{mkEntry("a"), mkEntry("b")})
	first := p.pick(nil)
	second := p.pick(nil)
...
func TestDailyCapResetsOnDateRollover(t *testing.T) {
	p := newPool("zen", "priority", 1, []keyFileEntry{mkEntry("a")})
	if got := p.pick(nil); got == nil {
...
func TestDailyCapPredicateCatchesMidDayCapLowering(t *testing.T) {
	...
	p := newPool("zen", "priority", 5, []keyFileEntry{mkEntry("a")})
	p.mu.Lock()
	...
	p2 := newPoolPreserving("zen", "priority", 3, []keyFileEntry{mkEntry("a")}, rawKeysMap(p))
...
func TestDailyCapCountSurvivesRebuild(t *testing.T) {
	...
	p.pick(nil)
	p.pick(nil)
	p2 := newPoolPreserving("zen", "priority", 2, []keyFileEntry{mkEntry("a")}, rawKeysMap(p))
	if got := p2.pick(nil); got != nil {
