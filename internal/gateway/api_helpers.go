...
func actorEmail(u *store.User) string {
	if u == nil {
		return "?"
	}
	return u.Email
...
// already written.
func (g *gateway) applyMutation(w http.ResponseWriter, user *store.User, action, conflictMsg string, write func() error) bool {
	if err := write(); err != nil {
		...
		case err == store.ErrBuiltinProvider:
