...
func (g *gateway) conf() *config.Config { return g.cfgPtr.Load() }
...
func (g *gateway) rs() *settings.RuntimeSettings { return g.rsPtr.Load() }

// newGateway is the store-less constructor used by tests: pools come from the
// config.KeyFile, provider resolution falls back to config builtins, and auth uses
// cfg.APIKeys. Signature and behavior are load-bearing for existing tests.
func newGateway(cfg *config.Config, rs *settings.RuntimeSettings, kf *config.KeyFile) *gateway {
	g := &gateway{
...
func NewGatewayFromStore(cfg *config.Config, rs *settings.RuntimeSettings, st *store.Store) (*gateway, error) {
	g := newGateway(cfg, rs, &config.KeyFile{})
	g.store = st
