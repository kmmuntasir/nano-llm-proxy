...
func newPool(name, mode string, dailyCap int, entries []config.KeyFileEntry) *Pool {
	if mode != "lru" {
		mode = "priority"
	}
...
func newPoolPreserving(name, mode string, dailyCap int, entries []config.KeyFileEntry, prev map[string]*KeyState) *Pool {
