...
// testCfg mirrors Bootstrap's expectations: zen/kilo base URLs to import.
...
// openTestStore is the gateway-side copy of the store test helper.
...
func testRuntime() *settings.RuntimeSettings { return settings.DefaultRuntimeSettings() }
...
func testGateway(t *testing.T, handler http.Handler) *gateway {
	...
	cfg := &config.Config{
		Port: 0, Bind: "127.0.0.1",
		Zen:  config.ZenProviderConfig{BaseURL: up.URL},
...
func TestRotation429FallsBackToNextKey(t *testing.T) {
	...
	rs := testRuntime()
	kf := &config.KeyFile{Zen: []config.KeyFileEntry{
		{Label: "a", Key: "sk-zen-a"}, {Label: "b", Key: "sk-zen-b"},
	}}
...
func TestAuthErrorDisablesKey(t *testing.T) {
	...
	rs := testRuntime()
	kf := &config.KeyFile{Kilo: []config.KeyFileEntry{{Label: "a", Key: "sk-a"}, {Label: "b", Key: "sk-b"}}}
	g := newGateway(cfg, rs, kf)
	rec := httptest.NewRecorder()
...
func TestFreeTierErrorFailsFastWithoutBurningPool(t *testing.T) {
	...
	rs := testRuntime()
	kf := &config.KeyFile{Zen: []config.KeyFileEntry{{Label: "a", Key: "sk-a"}, {Label: "b", Key: "sk-b"}, {Label: "c", Key: "sk-c"}}}
	g := newGateway(cfg, rs, kf)
	rec := httptest.NewRecorder()
	g.handleChat(rec, chatReq("zen", "mimo-test-free"))
	if rec.Code != 502 {
...
func TestPriorityModeSticksToFirstKey(t *testing.T) {
	...
	rs := testRuntime()
	kf := &config.KeyFile{Zen: []config.KeyFileEntry{{Label: "a", Key: "sk-zen-a"}, {Label: "b", Key: "sk-zen-b"}}}
	g := newGateway(cfg, rs, kf)
...
func TestAnthropicXApiKeyAuth(t *testing.T) {
	...
	rs := testRuntime()
	kf := &config.KeyFile{Kilo: []config.KeyFileEntry{{Label: "a", Key: "sk-a"}}}
	g := newGateway(cfg, rs, kf)
...
func TestUAVersionFloor(t *testing.T) {
	...
	for ua, want := range cases {
		if got := settings.UAVersionOK(ua); got != want {
			t.Errorf("settings.UAVersionOK(%q) = %v, want %v", ua, got, want)
		}
	}
}
...
func ckID(st *store.Store, plaintext string) int64 {
	keys, _ := st.AllClientKeys()
	for _, k := range keys {
		if k.KeyHash == store.HashSecret(plaintext) {
			return k.ID
		}
	}
	return 0
