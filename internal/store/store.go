...
func (s *Store) loadSettingsDoc(raw string) (*settings.RuntimeSettings, error) {
...
func (s *Store) LoadRuntimeSettings() (*settings.RuntimeSettings, error) {
...
func (s *Store) SaveRuntimeSettings(rs *settings.RuntimeSettings) error {
...
func (s *Store) UpdateSettings(mutate func(*settings.RuntimeSettings) *settings.RuntimeSettings) error {
	tx, err := s.db.Begin()
	if err != nil {
		return err
	}
...
// UsageEvent is one settled request, persisted for date-range reporting.
