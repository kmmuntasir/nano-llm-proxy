...
func (g *gateway) handleMessages(w http.ResponseWriter, r *http.Request) {
	...
	var hint string
	if ref.typ == "opencode" {
		hint = g.messagesViaZen(ref, w, r, chat, upstreamModel, clientWantsStream, start)
	} else {
		hint = g.messagesViaOpenAI(ref, w, r, chat, upstreamModel, clientWantsStream, start)
	}
	if hint != "" {
...
func (g *gateway) messagesViaOpenAI(ref providerRef, w http.ResponseWriter, r *http.Request, chat map[string]any, model string, stream bool, start time.Time) string {
	chat["stream"] = stream
...
func (g *gateway) messagesViaZen(ref providerRef, w http.ResponseWriter, r *http.Request, chat map[string]any, model string, stream bool, start time.Time) string {
	exclude := map[string]bool{}
	...
	for attempt := 0; attempt < g.rs().Retry.MaxKeysPerRequest; attempt++ {
		...
		if surface == "responses" {
			if stream {
				...
				writeAnthropicJSON(w, responsesSSEToAnthropicMessage(resp.Body, model))
			}
		} else if stream {
			a.start()
			streamChatSSEToAnthropic(a, resp.Body)
		} else {
			writeAnthropicJSON(w, chatSSEToAnthropicMessage(resp.Body, model))
		}
		g.recordActivity(r, ref, model, k.Hash, start, http.StatusOK, "")
		return ""
	}
