...
func (g *gateway) syncModelMeta(devURL string) ModelMetaSyncStatus {
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	defer cancel()
	st := ModelMetaSyncStatus{At: time.Now().Unix()}
...
func (g *gateway) mergeModelMeta(ctx context.Context, models map[string]modelsDevModel, st *ModelMetaSyncStatus) error {
	...
	for _, id := range ids {
		...
		current.Zen.ModelMeta[id] = ModelMeta{
	...
	st.OK = true
	status := *st
	if err := g.store.UpdateSettings(func(rs *RuntimeSettings) *RuntimeSettings {
...
func (g *gateway) failModelMetaSync(format string, args ...any) ModelMetaSyncStatus {
	st := ModelMetaSyncStatus{At: time.Now().Unix(), Error: strings.TrimSpace(fmt.Sprintf(format, args...))}
